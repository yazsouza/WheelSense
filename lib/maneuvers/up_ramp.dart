...
final upRampManeuver = Maneuver(
 ...
 evaluator: (List<WheelData> pool) {
    if (pool.length < 5) return TestEvaluation(0, ['Not enough data. Make sure you complete the ramp ascent.']);
    ...
    // 1. COMPLETELY WRONG WAY (Auto-Score 0)
    if (avgSpeed < -0.1) {
    ...
    const double rollbackDeadband = -0.3;
    ...
    // 3. INCONSISTENT SPEED (Capped at 30 pts)
    ...
    if (endIdx > startIdx) {
      ...
      for (final d in midPool) totalDeviation += (d.speedMS - midAvgSpeed).abs();
